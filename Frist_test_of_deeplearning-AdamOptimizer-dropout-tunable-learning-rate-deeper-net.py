...
data_folder = 'data_{0}_label_mass'.format(nsr)
h5_file_name = 'data_{0}_label_mass.h5'.format(nsr)
...
paths = '/work1/leo830227/GW_training_data_{0}/{1}_123/{2}'.format(training_data_folder, data_folder, h5_file_name) #data path
#print(paths)
...
save_folder_name = '1_train'
save_file_name = '1_train_0.1_PE'
"""
...
writer = tf.summary.FileWriter(
		'/home/leo830227/result_deeper_detction/tensorboard/0_train', sess.graph)
train_accuracy_tb_scalar = tf.summary.scalar('train_accuracy', train_accuracy_tb)
val_accuracy_tb_scalar = tf.summary.scalar('val_accuracy', val_accuracy_tb)
...
print('Test:')
for k in range(np.shape(rand_x)[0]):
	print('Mass Label: {0}. Prediction: {1}. Relative_loss: {2}.'.format(test_mass_label_np[k,:], result_preds_np[k,:], test_acc_np[k,:]))
"""
